...
export async function searchSrs(query: string): Promise<SupplierProductResult[]> {
  const q = query.trim();
  if (!q) return [];

  const apiKey = getSerpApiKey();
  const biased = `${q} SRS Building Products`;
  ...
  try {
    ...
    console.error("SerpApi SRS Building Products search failed:", err);
    return [];
  }
}
