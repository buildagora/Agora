 ...
 * Responsibilities:
 * - Read buyer intents
 * - Compare against request/order state
 * - Decide: auto-respond, update structured fields, or escalate to human
 * 
 * Implementation: Pure if/else logic (no LLMs)
 */
...
export function processBuyerMessage(
  ...
  // Gather context
  const context = gatherContext(message, sellerId);
 ...
 */
function gatherContext(message: Message, sellerId: string): AgentContext {
