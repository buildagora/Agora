  ...
  return (
    <div className="flex flex-1 px-6 py-8">
      <div className="w-full max-w-6xl mx-auto">
        ...
        <div className="mb-10">
          <Card className="border-zinc-200 shadow-md shadow-zinc-200/50">
            <CardContent className="p-8 sm:p-10">
              ...
              <div className="flex flex-wrap gap-3 items-stretch rounded-xl bg-zinc-50/80 p-4 border border-zinc-100">
                <select
                  required
                  value={selectedCategory}
                  onChange={(e) => setSelectedCategory(e.target.value)}
                  className="min-w-[180px] px-4 py-2.5 rounded-lg border border-zinc-300 bg-white text-black text-sm focus:outline-none focus:ring-2 focus:ring-slate-500 focus:border-transparent"
                  aria-label="Category"
                >
                ...
                <input
                  ...
                  className="flex-1 min-w-[200px] px-4 py-2.5 rounded-lg border border-zinc-300 bg-white text-black placeholder-zinc-500 text-sm focus:outline-none focus:ring-2 focus:ring-slate-500 focus:border-transparent"
        ...
        <div>
          <h2 className="text-lg font-semibold text-black mb-4">Recent Supplier Activity</h2>
          {activityLoading ? (
                ...
                const content = (
                  <>
                    <div className="flex items-start justify-between gap-3 mb-2">
                      <p className="font-semibold text-black">{item.supplierName}</p>
                      <span className="text-xs text-zinc-500 shrink-0">{item.timestamp}</span>
                    </div>
                    <p className="text-xs text-zinc-500 mb-2">{contextLabel}</p>
                    <p className="text-sm text-zinc-600 line-clamp-2">{item.messagePreview}</p>
                  </>
                );
                return (
                  <li key={i}>
                    {isClickable ? (
                      <button
                        ...
                        className="w-full text-left rounded-lg border border-zinc-200 bg-white p-4 shadow-sm hover:bg-zinc-50 hover:border-zinc-300 transition-colors cursor-pointer"
                      >
                        {content}
                      </button>
                    ) : (
                      <div className="w-full text-left rounded-lg border border-zinc-200 bg-white p-4 shadow-sm">
                        {content}
                      </div>
                    )}
                  </li>
