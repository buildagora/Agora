    ...
    try {
      ...
      const response = await fetch("/api/auth/sign-up", {
        ...
        body: JSON.stringify({
          ...
          role: "SELLER",
          categoriesServed, // Required for SELLER
          serviceArea: formData.businessAddress.trim() || undefined,
        }),
      });
      ...
      if (!response.ok) {
        ...
        if (errorData.error === "email_exists") {
          ...
          const errorMessage = errorData.message || errorData.error || "Failed to create account. Please try again.";
          console.error("[SELLER_SIGNUP_ERROR]", {
            status,
            error: errorData,
        ...
        const errorMessage = errorData.message || errorData.error || `Failed to create account (${status}). Please try again.`;
        setErrors({ 
          submit: errorMessage
        });
        setIsSubmitting(false);
