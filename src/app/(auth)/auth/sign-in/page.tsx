...
export default function SignInPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!userData.id || !userData.email || !userData.role) {
      console.error("[LOGIN_ERROR] User data missing required fields", userData);
    ...
    const { validateReturnTo, getDashboardForRole } = await import("@/lib/auth/routeIntent");
    
    // Helper function for default dashboard routing
    function getDefaultDashboardPath(user: typeof userData): string {
    ...
    if (returnToParam) {
      try {
        ...
        // Validate returnTo path for safety and role match
        if (validateReturnTo(decodedReturnTo, userData.role)) {
          redirectPath = decodedReturnTo;
        } else {
          // ReturnTo path doesn't match role or is unsafe - fall back to default dashboard
          redirectPath = getDefaultDashboardPath(userData);
        }
      } catch {
