...
import { useKeyboardAwareInset } from "@/lib/useKeyboardAwareInset";
import Sheet from "@/components/ui2/Sheet";
import BottomSheet from "@/components/ui2/BottomSheet";
import {
  getSortedThreads,
  createThread,
  getThread,
...
export default function BuyerAgentClient() {
  ...
  const bottomInsetPx = useKeyboardAwareInset();
  ...
  const isProcurementMode = ((rawCanonicalDraft as any)?.conversationMode) === "procurement" || 
                            (rawCanonicalDraft !== null && ((rawCanonicalDraft as any).conversationMode) !== "advice" && hasProcurementSignal(rawCanonicalDraft));
  ...
  // Lock body scroll on mobile for iOS keyboard handling
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col min-w-0 border-r border-zinc-200 dark:border-zinc-700">
        {/* Mobile Header */}
        <div className="md:hidden bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 px-4 py-3 flex items-center justify-between">
          <button
          ...
          <button
            ...
            className="p-2 -mr-2 text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-50 text-sm font-medium"
            aria-label="Details"
          >
            Details
          </button>
        ...
        {/* Desktop Header */}
        <div className="hidden md:block p-6 border-b border-zinc-200 dark:border-zinc-700">
          <div>
        ...
        <div className="flex-1 min-h-0 flex flex-col md:p-6">
          <div className="flex-1 min-h-0">
            <Chat
              ...
              draft={draft}
              onReviewAndSend={canConfirm ? handleCreateRequest : undefined}
      ...
      {/* Desktop Execution Panel - hidden on mobile */}
      <div className="hidden md:block w-96 p-6 overflow-y-auto">
        {draft ? (
          <ExecutionPanel
            draft={draft}
      ...
      {/* Mobile Bottom Sheet for Execution Panel */}
      <BottomSheet open={mobileDetailsOpen} onOpenChange={setMobileDetailsOpen}>
        <div className="p-6">
          {draft ? (
            <ExecutionPanel
              draft={draft}
