...
export function canonicalDraftToExecutionPanelDraft(
  ...
  const categoryLabel =
    ...
    (canonicalDraft.categoryId ? categoryIdToLabel[canonicalDraft.categoryId as keyof typeof categoryIdToLabel] ?? "" : "") ||
    "Materials";
...
export function canonicalDraftToRoutingDraft(
  ...
  // Routing contract prefers canonical categoryId.
  // DO NOT set `category` (legacy display label) here because the legacy union
  // does not include all of our labels and causes compile failures.
  return {
    id: draftId,
    title: canonicalDraft.jobNameOrPo || "",
    categoryId: canonicalDraft.categoryId || undefined,
    fulfillmentType: canonicalDraft.fulfillmentType || undefined,
    requestedDate: canonicalDraft.needBy || undefined, // legacy name
    location: canonicalDraft.deliveryAddress || undefined, // legacy name
    notes: canonicalDraft.notes || undefined,
    jobNameOrPo: canonicalDraft.jobNameOrPo || undefined,
    lineItems: canonicalDraft.lineItems || undefined,
  };
}
